...
## Architecture Traps (check before writing)

<!-- Verify these common pitfalls before implementing -->
- [ ] **Image chat renderer**: If touching image chat messages → use `renderIchatMessage()` (~line 14812), NOT `renderImageChatThreadMessages()` (~line 5489, legacy/do not modify)
- [ ] **postImageState IPC**: sends LITE resultBatches (src stripped). Full src only in `image:result`. Don't assume src is present in `imageState.resultBatches`.
- [ ] **Build step**: `index.html` changes require `npm run build:electron` to take effect — tsc alone is not enough.
