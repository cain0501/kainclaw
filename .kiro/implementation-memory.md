...
- 2026-04-28 installed-skills implementation memory:
  ...
  - The current bilingual renderer surface includes approval / AskUserQuestion dialogs, `/freeze`, `/debug ask-user-question [single|multi]`, and the highest-visibility settings-page language/model/image-model labels. Use the same language table when widening this scope instead of adding fresh inline literals.
  - Keep the Electron language selector in the Advanced settings section, not in the provider/model section. Treat that location as part of the product contract unless the information architecture is intentionally redesigned.
