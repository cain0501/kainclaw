...
### Component visual quality (CSS execution)

- [ ] **Primary CTA button is visually dominant.** `btn-primary` must use `var(--accent)` fill, font-weight ≥ 600, min-height 48px. A bare rectangle with text is a failure.
- [ ] **Secondary actions are visually distinct from primary.** If both CTA and secondary button exist on screen, they must differ in fill/stroke/weight — not just label text.
- [ ] **Interactive selector states are styled (3 states minimum).** Size grids, color pickers, tab selectors: selected = accent fill or accent border; available = subtle border, muted background; sold-out = `opacity: 0.35` + `text-decoration: line-through`. All three states must be visually unambiguous.
- [ ] **Price display hierarchy is enforced.** Current price: `font-size ≥ 24px`, `font-weight: 700`, `color: var(--accent)`. Original/crossed-out price: `font-size` 1 step smaller, `text-decoration: line-through`, `color: var(--muted)`. Flat same-size same-weight prices are a failure.
- [ ] **Section cards use the template's `.card` class or equivalent.** Do not use raw `<div style="background: ...">` inline for card containers — use `.card`, `.card.flat`, or define a named class.
- [ ] **Trust / badge / tag elements are visually differentiated.** Platform guarantee, flash-delivery, return-policy tags must use `var(--accent)` tint background or a border — not plain inline text.
