...
The **class inventory below is the design system** — always use it. The archetypes are optional shortcuts for common patterns.

- **If your screen matches an archetype** → paste it, swap copy, done.
- **If no archetype matches** → build the content freely using the class inventory. Do not force an unrelated archetype onto a different screen type. A product detail page, an e-commerce homepage, a checkout flow — none of these need an archetype; they need the right classes assembled for the brief.

**Rule**: structure comes from the brief, visual quality comes from the class system. Never invent inline `style=""` CSS for layout when a class covers it.

## Class inventory — use these for ALL content, archetype or not

| Class | What it looks like |
|---|---|
| `card` | White/surface rounded card with border |
| `card.accent` | Card with accent-tinted background |
| `card.flat` | No border, subtle background only |
| `list-row` | Full-width row ≥48px, left content + optional right action |
| `avatar` | Circular image placeholder, default 40px |
| `ph-img` | Rectangular image placeholder (add `.wide`, `.square`, `.tall` for ratio) |
| `grid-2` / `grid-3` | 2 or 3-column equal grid |
| `row` | Horizontal flex, gap 12px, items centered |
| `row-between` | Row with space-between — section header pattern |
| `stack` | Vertical flex, gap 16px |
| `pad` | Inline padding 20px both sides |
| `btn-primary` | Accent fill, full-width, 48px, font-weight 600 |
| `btn-secondary` | Outline/surface, same size |
| `tag` | Small inline label, surface background, rounded |
| `pill` | Accent-tinted rounded pill — use for status or category |
| `num` | Mono font — prices, counts, durations |
| `h2` / `h3` | Display font headings |
| `meta` | Small muted label text |
| `progress` | Thin accent progress bar |

**If you need a class not listed**: define it once in a `<style>` block inside `<main>`, using `var(--bg)` / `var(--accent)` / `var(--fg)` tokens only. Never hardcode hex colors.

**HARD RULE — buttons**: Every primary action MUST be `<button class="btn-primary">`. Every secondary action MUST be `<button class="btn-secondary">`. Writing `<button style="...">` or a bare `<button>` with no class is forbidden. The template already styles these — just use the class.

## Pre-flight

1. Read `template.html` through the `<style>` block — all classes above are defined there.
2. The Dynamic Island, status bar, home indicator, and tab bar are already in the template — do not re-implement them.
3. If the screen has a persistent nav (feed, profile, explore), keep `<nav class="tabbar">`. For detail, checkout, onboarding — remove it.
