...
- No AI-illustrated human faces or generic stock-photo descriptions. Images and product photos are required - use `https://picsum.photos/seed/{slug}/{w}/{h}` or `https://placehold.co/{w}x{h}` as placeholder src values. Never leave an image area empty or replaced by a grey box.
