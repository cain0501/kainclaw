...
const ELECTRON_SHELL_SURFACE_TEXT_PAIRS = [
  ...
  ["你想怎么处理？", "What would you like to do?"],
  ["要将这个 GitHub 仓库安装为 MCP 服务器吗？", "Do you want to install this GitHub repository as an MCP server?"],
  ["安装为 MCP", "Install as MCP"],
  ["安装前先检查其中可机器读取的 MCP 配置。", "Inspect its machine-readable MCP configuration before installation."],
  ["继续对话", "Continue chatting"],
  ["将它作为普通仓库讨论继续。", "Keep this as a normal repository discussion."],
  ["这个 GitHub 仓库中没有找到受支持的 MCP 声明，未安装任何配置。", "I could not find a supported MCP declaration in this GitHub repository. No configuration was installed."],
  ["已取消 MCP 安装，未修改任何配置。", "MCP installation cancelled. No configuration was changed."],
  ["发现 MCP 服务器", "MCP server found"],
  ["选择一个服务器", "Choose one server"],
  ["你想安装哪个 MCP 服务器？", "Which MCP server would you like to install?"],
  ["确认 MCP 安装", "Confirm MCP installation"],
  ["安装前检查", "Review before installing"],
  ["安装并信任", "Install and trust"],
  ["取消", "Cancel"],
  ["不要写入或信任此 MCP 配置。", "Do not write or trust this MCP configuration."],
  ["请提供公开的 GitHub 仓库 URL。", "Provide a public GitHub repository URL."],
  ["请提供 GitHub 仓库 URL 或仓库 tree URL。", "Provide a GitHub repository URL or a repository tree URL."],
  ["GitHub 仓库检查当前不可用。", "GitHub repository inspection is unavailable."],
  ["GitHub 仓库检查拒绝了重定向。", "GitHub repository inspection rejected a redirect."],
  ...
  ["设计", "Design"],
  ["作品库", "Library"],
  ["图像是素材流，设计是作品流。统一壳层，不统一主对象。", "Images are asset flows; designs are work flows. One shell, separate primary objects."],
  ["快速操作", "Quick actions"],
  ["打开作品库", "Open library"],
  ["当前作品画布", "Current work canvas"],
  ["当前上下文", "Current context"],
  ["快捷跳转", "Shortcuts"],
  ["回到设计对话", "Back to design chat"],
  ["当前作品版本", "Current work versions"],
  ["返回对话", "Back to chat"],
  ["编辑中·项目", "Editing - project"],
  ...
  ["描述你想要的图像...", "Describe the image you want..."],
  ["历史提示词 ▾", "Prompt history ▾"],
  ["参考图（可选，最多 4 张）", "Reference images (optional, up to 4)"],
  ["+ 上传参考图", "+ Upload reference image"],
  ["✦ AI 反推描述", "Infer description"],
  ["未上传参考图，最多 4 张。", "No reference images uploaded. Up to 4 images."],
  ["比例", "Aspect ratio"],
  ["方形", "Square"],
  ["横版", "Landscape"],
  ["竖版", "Portrait"],
  ...
  ["生成预览", "Preview"],
  ["我的作品", "My works"],
  ["图像生成预览", "Image generation preview"],
  ["先在左侧输入图像描述，再生成预览结果。", "Enter an image description on the left, then generate preview results."],
  ["设计作品库", "Design library"],
  ["图片素材库", "Image asset library"],
  ["设计需求", "Design request"],
  ["在这里填写设计需求…", "Describe the design request here..."],
  ...
  ["信息图", "Infographic"],
  ["动效页", "Animated page"],
  ["小红书图文", "Xiaohongshu carousel"],
  ["邮件模板", "Email template"],
  ["移动端 App", "Mobile app"],
  ["杂志海报", "Magazine poster"],
  ["数据看板", "Dashboard"],
  ["文档报告", "Document report"],
  ["定价对比页", "Pricing comparison page"],
  ["产品落地页", "Product landing page"],
  ["主要给谁看？（可选）", "Who is the audience? (optional)"],
  ["如：消费者、投资人、内部团队…", "Example: consumers, investors, internal teams..."],
  ["拖拽调整左栏宽度", "Drag to resize the left panel"],
  ["替换目标：项目 · 元素", "Replacement target: project - element"],
  ["取消替换", "Cancel replacement"],
  ["查看与操作", "View and actions"],
  ["查看", "View"],
  ["选择", "Select"],
  ["调节", "Tune"],
  ["评审", "Review"],
  ...
  ["导出 PPTX", "Export PPTX"],
  ["导出 ZIP", "Export ZIP"],
  ["暂无设计内容", "No design content yet"],
  ["从我的作品选择", "Select from my works"],
  ["应用改写", "Apply rewrite"],
  ["当前作品", "Current work"],
  ["当前目标", "Current target"],
